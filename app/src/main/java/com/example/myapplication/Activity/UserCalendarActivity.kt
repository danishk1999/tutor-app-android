...
class UserCalendarActivity : AppCompatActivity() {
    ...
    private fun fetchUserAppointments() {
        ...
        database.orderByChild("studentId").equalTo(studentId.toDouble()).addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@UserCalendarActivity, "Failed to fetch appointments", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun updateCalendarDecorator() {
        val calendarView: MaterialCalendarView = findViewById(R.id.calendarView)
        val appointmentDecorator = AppointmentDecorator(this, userAppointments)
        ...
        database.orderByChild("date").equalTo(formattedDate).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    if (!appointmentFound) {
                        appointmentDetailsView.visibility = View.GONE
                        Toast.makeText(this@UserCalendarActivity, "No appointments on this date", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    appointmentDetailsView.visibility = View.GONE
                    Toast.makeText(this@UserCalendarActivity, "No appointments on this date", Toast.LENGTH_SHORT).show()
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@UserCalendarActivity, "Failed to fetch appointment details", Toast.LENGTH_SHORT).show()
            }
        })
